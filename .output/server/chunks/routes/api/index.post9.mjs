...
import { n as notifyUser } from '../../_/notification.service.mjs';
...
const index_post = defineEventHandler(async (event) => {
  ...
  const order = await prisma.$transaction(async (tx) => {
    ...
    const orderStatus = hasDistributor ? "ASSIGNED" : "COMPLETED";
    const created = await tx.salesOrder.create({
      data: {
        ...
        deliveryResult: hasDistributor ? "NONE" : "FULL",
        completedAt: hasDistributor ? null : /* @__PURE__ */ new Date(),
    ...
    if (hasDistributor && created.assignedDistributorId) {
      await notifyUser(created.assignedDistributorId, {
        ...
        data: {
          ...
          customerName: (_a2 = created.customer) == null ? void 0 : _a2.name
        }
      });
    }
    if (!hasDistributor) {
