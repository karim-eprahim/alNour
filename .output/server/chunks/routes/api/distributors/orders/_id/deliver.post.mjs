...
const deliver_post = defineEventHandler(async (event) => {
  ...
  if (result === "CANCELLED") {
    const updated = await prisma.$transaction(async (tx) => {
      const order2 = await tx.salesOrder.update({
        where: { id },
        data: {
      ...
      await tx.deliveryTracking.updateMany({
        ...
        data: { status: "CANCELLED", endedAt: /* @__PURE__ */ new Date(), lastUpdatedAt: /* @__PURE__ */ new Date() }
      });
      return order2;
    });
    return { order: { id: updated.id, status: updated.status, deliveryResult: updated.deliveryResult } };
  }
  const delivered = result === "FULL" ? order.items.map((item) => ({
  ...
  if (result === "PARTIAL" && delivered.every((item) => item.quantity === orderedByProduct.get(item.productId))) {
    throw createError({ statusCode: 400, statusMessage: "Delivered quantities match the full order. Use FULL instead." });
  }
  const invoiceItems = delivered.filter((item) => item.quantity > 0);
  const totalAmount = invoiceItems.reduce((sum, item) => sum + item.quantity * unitPriceByProduct.get(item.productId), 0);
  ...
  const completed = await prisma.$transaction(async (tx) => {
    ...
    await tx.deliveryTracking.updateMany({
      ...
      data: { status: "COMPLETED", endedAt: /* @__PURE__ */ new Date(), lastUpdatedAt: /* @__PURE__ */ new Date() }
    });
