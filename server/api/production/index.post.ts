export default defineEventHandler(async (event) => {
  ...
  if (!body.warehouseId || !body.consumptions?.length || !body.outputs?.length) {
  ...
  const batch = await prisma.$transaction(async (tx) => {
    ...
    const consumptionsData = await Promise.all(
      body.consumptions.map(async (c: any) => {
        const qty = parseFloat(c.quantity) || 0
        const product = await tx.product.findUnique({ where: { id: c.productId } })
        const unitCost = product?.purchaseCost ? product.purchaseCost.toNumber() : 0
    ...
    const outputsData = body.outputs.map((o: any) => {
      const qty = parseFloat(o.quantity) || 0
      const waste = parseFloat(o.waste) || 0
      return { productId: o.productId, quantity: qty, waste }
    })

    const totalOutputQty = outputsData.reduce((sum: number, o: any) => sum + o.quantity, 0)
